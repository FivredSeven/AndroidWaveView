...
public class CheapWAV extends CheapSoundFile {
    ...
    public void ReadFile(File inputFile)throws java.io.FileNotFoundException,java.io.IOException {
        ...
        while (mOffset + 8 <= mFileSize) {
            ...
            if (stream.read(chunkHeader, 0, 8) < 0){
                throw new java.io.IOException("Can not read chunkHeader ");
            }
            mOffset += 8;
            int chunkLen =
            ...
            if (chunkHeader[0] == 'f' &&
                ...
                if (stream.read(fmt, 0, chunkLen) < 0){
                    throw new java.io.IOException("Can not read chunkLen ");
                }
                mOffset += chunkLen;
                ...
                while (i < chunkLen) {
                   ...
                   /* try {
                    ...
                    if (stream.read(oneFrame, 0, oneFrameBytes) < 0){
                        throw new java.io.IOException("Can not read oneFrame ");
                    }
                ...
                if (stream.skip(chunkLen) < 0){
                    throw new java.io.IOException("Can not skip chunkLen ");
                }
                mOffset += chunkLen;
            }
        }
    ...
    public void WriteFile(File outputFile, int startFrame, int numFrames)
        ...
        for (int i = 0; i < numFrames; i++) {
            ...
            if (skip > 0) {
                if (in.skip(skip) < 0){
                    throw new java.io.IOException("Can not skip skip ");
                }
                pos += skip;
            }
