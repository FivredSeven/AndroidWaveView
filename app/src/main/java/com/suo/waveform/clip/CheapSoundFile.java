...
import com.suo.waveform.util.FileUtil;
...
public class CheapSoundFile {
    public static String REAL_EXTNAME = "mp3";
...
//             CheapAMR.getFactory(),
    ...
    public static CheapSoundFile create(String fileName, ProgressListener progressListener)
        ...
        if (FileUtil.isTempCachePath(name)) {
            components[components.length - 1] = REAL_EXTNAME;
        }
    ...
    public String computeMd5OfFirst10Frames() throws java.io.FileNotFoundException,
        ...
        try{
            ...
            for (int i = 0; i < numFrames; i++) {
                ...
                if (skip > 0) {
                    int skipLen = skip;
                    while(skipLen > 0){
                        long amt = in.skip(skipLen);
                        if (amt == -1){
                            throw new RuntimeException(mInputFile + ": unexpected EOF");
                        }
                        skipLen -= amt;
                    }
                    pos += skip;
                ...
                int readLen = len;
                while (readLen > 0){
                    long rmt = in.read(buffer, 0, readLen);
                    if (rmt == -1){
                        throw new RuntimeException(mInputFile + ": unexpected EOF");
                    }
                    readLen -= rmt;
    ...
    public void WriteFileTemp(File outputFile, int startFrame, int numFrames, boolean is,
                              boolean out) throws java.io.IOException {
