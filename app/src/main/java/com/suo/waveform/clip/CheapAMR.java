...
public class CheapAMR extends CheapSoundFile {
    ...
    public void ReadFile(File inputFile) throws java.io.FileNotFoundException, java.io.IOException {
        ...
        if (stream.read(header, 0, 6) < 0){
            throw new java.io.IOException("Can not read header");
        }
        mOffset += 6;
        if (header[0] == '#' && header[1] == '!' &&header[2] == 'A' &&header[3] == 'M' &&header[4] == 'R' &&header[5] == '\n') {
            parseAMR(stream, mFileSize - 6);
        }

        if (stream.read(header, 6, 6) < 0){
            throw new java.io.IOException("Can not read header2");
        }
        mOffset += 6;
        if (header[4] == 'f' && header[5] == 't' &&header[6] == 'y' &&header[7] == 'p' &&header[8] == '3' &&header[9] == 'g' &&header[10] == 'p' &&header[11] == '4') {
            int boxLen =
                    ((0xff & header[0]) << 24) |
                            ((0xff & header[1]) << 16) |
                            ((0xff & header[2]) << 8) |
            ...
            if (boxLen >= 4 && boxLen <= mFileSize - 8) {
                if (stream.skip(boxLen - 12) < 0){
                    throw new java.io.IOException("Can not skip boxLen");
                }
                mOffset += boxLen - 12;
            }
    ...
    private void parse3gpp(InputStream stream, int maxLen)throws java.io.IOException {
        ...
        if (stream.read(boxHeader, 0, 8) < 0){
            throw new java.io.IOException("Can not read boxHeader");
        }
        mOffset += 8;
...
//        }
    ...
    int parseAMRFrame(InputStream stream, int maxLen, int[] prevEner) throws java.io.IOException {
        ...
        if (stream.read(frameTypeHeader, 0, 1) < 0){
            throw new java.io.IOException("Can not read frameTypeHeader");
        }
        mOffset += 1;
        ...
        if (stream.read(v, 0, blockSize) < 0){
            throw new java.io.IOException("Can not read blockSize");
        }
        mOffset += blockSize;
    ...
    public void WriteFile(File outputFile, int startFrame, int numFrames)
        ...
        for (int i = 0; i < numFrames; i++) {
            ...
            if (skip > 0) {
                if (in.skip(skip) < 0){
                    throw new java.io.IOException("Could not skip stream");
                }
                pos += skip;
            }
            if (in.read(buffer, 0, len) < 0){
                throw new java.io.IOException("Can not read buffer");
            }
